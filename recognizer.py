...
# UPI Payment Redirection (Modified to Print UPI Link for Testing)
def initiate_upi_payment(upi_id, amount, note):
    upi_link = f"upi://pay?pa={upi_id}&pn=Payment&tn={note}&am={amount}&cu=INR"
    
    # Print the UPI link to the console for manual testing
    print(f"Generated UPI Link: {upi_link}")
    
    # Optionally, open the UPI link for real payment if needed (uncomment the line below)
    # webbrowser.open(upi_link)

    # Show the UPI link in a messagebox
    root = tk.Tk()
    root.withdraw()
    messagebox.showinfo("UPI Link Generated", f"Generated UPI Link:\n{upi_link}")
    root.destroy()
